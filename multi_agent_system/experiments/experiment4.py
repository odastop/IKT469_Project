# Experiment 4: Multi-hop QA pipeline without question decomposition and broad query expansion.
...
class BM25Retriever:
    def __init__(self, corpus: Dict[str, List[str]], k1: float = 1.5, b: float = 0.75) -> None:
        self.corpus = corpus
        self.k1 = k1
        self.b = b
    ...
    def search(self, query: str, top_k: int = 10) -> List[RetrievedDocument]:
        ...
        return [RetrievedDocument(title, self.corpus[title], score) for title, score in ranked]


# Local Ollama API client
class OllamaClient:
    def __init__(self, model: str = "mixtral", host: str = "http://127.0.0.1:11434") -> None:
        self.model = model
...
def extract_entities(question: str) -> List[str]:
    # Extract quoted and capitalized entities
    ...
    # Prefer multi-word entities over overlapping single words
    multi = [e for e in unique(entities) if " " in e]
    single = [e for e in unique(entities) if " " not in e]
    single = [e for e in single if not any(e.lower() in m.lower() for m in multi)]

    return unique(multi + single)


def infer_relation_terms(question: str) -> List[str]:
    # Map question keywords to retrieval terms
    q = question.lower()
    terms = []
...
def question_type(question: str) -> str:
    # Classify question from its opening words
...
def build_corpus_from_split(split: str, max_examples: int | None = None) -> Dict[str, List[str]]:
    ...
    # Extract unique context documents
...
def load_or_build_corpus(
    ...
    # Reuse cached corpus when available
    ...
    # Save corpus cache for later runs
...
def parse_bullets(text: str) -> List[str]:
    # Parse bullet-point output from the LLM
    return unique(
        [
            line.strip()[2:].strip()
...
def analyze_question(llm: OllamaClient, question: str, entities: List[str]) -> List[str]:
    # Generate retrieval-focused subquestions
    prompt = f"""
...
"""
    ...
    # Fallback if no valid bullets are returned
    fallback = [question]
...
# Select supporting evidence documents
def selector_agent(
    llm: OllamaClient,
    question: str,
    ...
    # Ask LLM to choose relevant evidence
    prompt = f"""
...
"""
    ...
    # Use heuristic fallback if selection fails
    if not ids:
...
def adder_agent(
    ...
    # Ask LLM to add missing evidence
    prompt = f"""
You are AdderAgent.
...
"""
    ...
    # Keep only newly selected documents
    added = []
    for i in ids:
        doc = all_docs[i]
...
def parse_answer_text(raw: str) -> str:
    # Extract answer from expected LLM output
...
def clean_prediction(pred: str, qtype: str) -> str:
    # Normalize final prediction text
...
def answer_agent(
    ...
    # Ask LLM to answer using selected evidence only
    prompt = f"""
...
def fallback_select(question: str, docs: List[RetrievedDocument], max_docs: int = 8) -> List[int]:
    # Heuristic fallback document selection
...
# Restricted retrieval without broad entity expansion
def retrieve_candidates(
    ...
    for entity in entities:
        # Ablation: use only focused entity-relation queries
        for term in relation_terms:
...
# Execute full QA pipeline for one question
def run_pipeline(
    question: str,
    retriever: BM25Retriever,
    llm: OllamaClient,
    ...
    # Ablation: remove question decomposition
...
def normalize_answer(s: str) -> str:
    # Normalize text before answer comparison
...
def title_recall(retrieved_titles: List[str], gold_titles: List[str]) -> float:
    # Compute recall of supporting document titles
...
# Evaluate pipeline on HotpotQA examples
def run_eval(
    retriever: BM25Retriever,
    llm: OllamaClient,
    eval_n: int,
    ...
    for i, item in enumerate(dataset):
        ...
        retrieval_recall = title_recall(result.retrieved_titles[:30], supporting_titles)
        evidence_recall = title_recall(result.selected_titles, supporting_titles)
    ...
    avg_retrieval_recall = sum(r["retrieval_recall"] for r in records) / max(len(records), 1)
    avg_evidence_recall = sum(r["evidence_recall"] for r in records) / max(len(records), 1)
    avg_runtime = sum(r["runtime"] for r in records) / max(len(records), 1)
    avg_subquestions = sum(r["subquestion_count"] for r in records) / max(len(records), 1)
    avg_selected_docs = sum(r["selected_count"] for r in records) / max(len(records), 1)
    avg_retrieved_docs = sum(r["retrieved_count"] for r in records) / max(len(records), 1)
