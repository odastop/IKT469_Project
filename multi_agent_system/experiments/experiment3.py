# Experiment 3: Multi-hop QA pipeline with restricted query expansion.
# This ablation removes broad entity expansion queries and keeps only entity-relation retrieval queries.
...
def extract_entities(question: str) -> List[str]:
    # Extract named entities from quoted and capitalized text
...
def parse_bullets(text: str) -> List[str]:
    # Parse bullet-point output from LLM prompts
    return unique(
        [
            line.strip()[2:].strip()
...
def heuristic_score_doc(
    ...
    # Score documents using entity, relation and token overlap
...
def answer_agent(
    ...
    # Generate final answer from selected evidence
    qtype = question_type(question)
...
def retrieve_candidates(
    ...
    # Restricted query expansion ablation
    relation_terms = infer_relation_terms(question)
    ...
    for entity in entities:
        # Keep only focused entity + relation-term queries.
        # Remove broad generic expansions like biography, born, country, etc.
        for term in relation_terms:
...
def run_pipeline(
    ...
    # Run full QA pipeline for one question
    start = time.time()
