# Experiment 1: Full multi-agent multi-hop QA pipeline.
# Includes BM25 retrieval, question decomposition, entity/relation query expansion, heuristic reranking, evidence selection, evidence refinement, and answer generation.
...
class BM25Retriever:
    def __init__(self, corpus: Dict[str, List[str]], k1: float = 1.5, b: float = 0.75) -> None:
        self.corpus = corpus  # Document corpus
        self.k1 = k1  # BM25 term frequency parameter
        self.b = b  # BM25 document length normalization parameter

        self.doc_len: Dict[str, int] = {}  # Document lengths
        self.inverted: Dict[str, List[Tuple[str, int]]] = defaultdict(list)  # Inverted index
        ...
        self.n_docs = len(corpus)  # Total number of documents
        self.avgdl = total_len / max(self.n_docs, 1)  # Average document length

    def search(self, query: str, top_k: int = 10) -> List[RetrievedDocument]:
        query_tokens = tokenize(query)
        scores: Dict[str, float] = defaultdict(float)

        # Compute BM25 scores for matching documents
...
class OllamaClient:
    def __init__(
        ...
        self.host = host.rstrip("/")  # Ollama API host
...
def infer_relation_terms(question: str) -> List[str]:
    ...
    # Map question keywords to related retrieval terms
    mapping = {
    ...
    # Add relation terms when their trigger word appears in the question
...
def question_type(question: str) -> str:
    ...
    # Classify question type from the opening words
...
def build_corpus_from_split(split: str, max_examples: int | None = None) -> Dict[str, List[str]]:
    # Load HotpotQA split from HuggingFace
    dataset = load_dataset("hotpotqa/hotpot_qa", "fullwiki", split=split)

    corpus: Dict[str, List[str]] = {}

    # Extract unique documents from the dataset contexts
...
def merge_corpora(*corpora: Dict[str, List[str]]) -> Dict[str, List[str]]:
    ...
    # Merge documents without duplicating titles
...
def load_or_build_corpus(
    ...
    # Optionally include validation documents in the corpus
    ...
    # Save corpus cache for faster later runs
...
def analyze_question(llm: OllamaClient, question: str, entities: List[str]) -> List[str]:
    # Ask the LLM to decompose the question into retrieval subquestions
    prompt = f"""
...
"""
    ...
    # Fallback if the LLM does not return valid bullets
    fallback = [question]
...
def parse_doc_ids(text: str, max_id: int) -> List[int]:
    ...
    # Extract document IDs returned by the LLM
...
def heuristic_score_doc(
    ...
    # Boost documents matching extracted entities
    for entity in entities:
        e = entity.lower()
    ...
    # Boost documents containing relation terms
    for term in relation_terms:
        if term.lower() in text:
            score += 10

    # Add small score for token overlap with the question
...
def rerank_candidates(
    ...
    # Score and sort retrieved documents
    scored = [
...
"""
    ...
    # Use heuristic fallback if the LLM returns no valid IDs
    if not ids:
...
def adder_agent(
    ...
    # Ask the LLM to add missing supporting documents
    prompt = f"""
You are AdderAgent.
...
def parse_answer_text(raw: str) -> str:
    ...
    # Fallback to first line if no ANSWER field is found
...
def answer_agent(
    ...
    # Ask the LLM to answer from selected evidence only
    prompt = f"""
...
def fallback_select(question: str, docs: List[RetrievedDocument], max_docs: int = 8) -> List[int]:
    ...
    # Heuristic document selection if LLM selection fails
    for i, doc in enumerate(docs):
        title = doc.title.lower()
        text = (doc.title + " " + " ".join(doc.sentences[:8])).lower()
...
def retrieve_candidates(
    ...
    # Build search queries from the question, entities and subquestions
    queries = []
    queries.append(question)
    queries.extend(entities)
    queries.extend(subquestions)

    # Add broader entity-based search queries
    for entity in entities:
        queries.extend(
            [
                entity,
    ...
    # Retrieve documents and remove duplicates by title
...
def run_pipeline(
    ...
    # Extract entities and generate subquestions
    entities = extract_entities(question)
    subquestions = analyze_question(llm, question, entities)
...
def title_recall(retrieved_titles: List[str], gold_titles: List[str]) -> float:
    # Compute supporting document recall
...
def run_eval(
    ...
    # Run evaluation on HotpotQA examples
    for i, item in enumerate(dataset):
        if i >= eval_n:
            break
...
def main() -> None:
    ...
    # Build retriever and LLM client
